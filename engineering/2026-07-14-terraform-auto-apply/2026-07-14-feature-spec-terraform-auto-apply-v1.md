...
|---------|-------|----------|--------|:--------:|
| 2026-07-14T19:30Z | 2026-07-14T19:30Z | ~0 (single authoring pass) | n/a (runtime usage not exposed for this pass) | 1 |
...
4. **firebase-dev**: on-merge auto, fast-path (no reviewer gate) → plan + apply. `⟶ D-2 (Q-4)`
5. **mono-root & firebase-prod**: the reusable apply workflow runs in the trusted context (Phase-1 mono-root via `workflow_dispatch`; end-state on-merge). A **plan job** runs `terraform plan -out=tfplan` on the target SHA, **seals `tfplan` with the SOPS age key**, uploads it as an *encrypted* artifact, and posts a **sanitized** plan summary. The **apply job** targets a GitHub **Environment** with a **required reviewer**; on approval it decrypts the sealed plan on the trusted runner and runs `apply tfplan`. `⟶ D-2, D-3, D-4, D-5`
...
#### BR-004 ⟶ D-4
Applied == approved is **content-equality on the post-squash SHA**, enforced by a same-target `plan -out=tfplan` → **age-seal** → approval-gated `apply tfplan`. No re-plan after approval. Terraform aborts on a stale plan. The sealed plan is decrypted only on the trusted runner; a plaintext plan is never persisted.
#### BR-005 ⟶ D-5
...
- [ ] Create + install the GitHub App; store `app_id`/`installation_id`/private key + age key in SOPS+age / Environment secrets (out-of-band). **TODO(A-3):** finalize App permission set. `⟶ D-5`
...
**Tasks:**
- [ ] `_tf-apply.yml` reusable workflow: WIF auth, App-token mint, SOPS decrypt, preflight, `plan -out`, age-seal, encrypted artifact, sanitized summary, Environment gate, decrypt, `apply tfplan`, outcome reporting; `concurrency` per prefix, `cancel-in-progress:false`; `TF_APPLY_ENABLED` guard. `⟶ D-4, D-5, D-6, D-8, D-9, D-10, D-11`
...
- **US-3 (gated):** firebase-prod dispatch → apply job waits at the `production` gate; approve → applies; assert an unapproved run performs zero apply and acquires no lock. `⟶ D-2`
