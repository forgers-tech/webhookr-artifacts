...
> **⚠️ Post-implementation drift note (2026-07-15) — amendment, no reopen.** During rollout, `artifact-reconciliation` (RR-1) found that **D-2's end-state reviewer gate and BR-015 condition 3** ("the reviewer approves the saved plan") are **not achievable on the current GitHub plan** — required-reviewers on a *private*-repo Environment need GitHub **Team+** (a 422 "billing plan"). The operator ratified **option (a)**: the `production` Environment carries only a `main`-only branch policy (no reviewer); the human control for gated roots is the **manual `workflow_dispatch`**, with the age-sealed saved plan still guaranteeing `applied == approved` and the no-destroy / clean-baseline guards + disable switch as backstops (consistent with **R-8**). Options **(b)** upgrade to Team and **(c)** a two-dispatch plan→apply flow are **deferred** (not rejected). **This is a decision the operator made and owns; per their call the council was not reopened** — the authoritative as-built + rationale live in **ADR-008 (`../2026-07-15-terraform-cd-pipeline/`), §As-built**. Read D-2 / BR-015-3 below as *superseded by that note* for the reviewer-gate mechanism only; every other `D-#` stands as written. Pipeline shipped + proven end-to-end 2026-07-15 (all three targets, incl. a real mono-root change apply).
