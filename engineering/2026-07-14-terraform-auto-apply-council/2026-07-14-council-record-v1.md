...
|---------|-------|----------|--------|:--------:|
| 2026-07-14T18:30Z | 2026-07-14T18:52Z | ~22 min | ≈898K (specialist executions; Historian Explore + Chair turns not separately metered → partial) | 1 |
...
- **New (Round-2 verified):**
  ...
  - **NF-C.** The project-memory `GITOPS_DEPLOY_PAT` shadow-override incident was **not found in-repo** by the Historian → treated as **unverified**; no control is built on it. The real, repo-visible bootstrap hazards are `backend.tf` (bucket bootstrapped out-of-band) and `github_secrets.tf` self-reference.
...
- **D-4** | **applied == approved = content-equality on the post-squash SHA**, via same-runner `plan -out` → Environment approval → `apply tfplan` in **one job**; the saved plan **never** persisted as an artifact; **no re-plan after approval** | US-4,US-7 | NF-B, C2 | re-plan-at-apply; plan-artifact hand-off (X-4) | requires the approval to hold mid-job (Q-1) | Architect
- **D-5** | **Credentials:** WIF/OIDC for GCS backend (no SA key); **ephemeral GitHub App installation token** for admin:org (retires static PAT); third-party OIDC-less secrets as **Environment-scoped** secrets, step-level, SHA-pinned actions, GitHub-hosted **ephemeral** runners | US-3,US-5 | F5, F7, OIDC fact, Historian P0.1 | static admin:org PAT (X-3); persistent self-hosted runner (X-2) | third-party crown-jewels still long-lived somewhere | Security/DevOps
...
- **A-3** | `integrations/github` provider supports GitHub App installation-token auth for the admin:org operations | Security/Architect | spike | Open | App-token path infeasible → DA's full-P0.1-remediation-first fallback applies
- **A-4** | A GH Environment approval can hold **mid-job** so one job spans plan→approve→apply keeping the `tfplan` in memory | Architect | workflow spike | Open (**→ Q-1**) | hard applied==approved (D-4) downgrades to human-review-only
...
- **Q-1** | Can a GH Environment approval hold mid-job so plan→approve→apply keeps the `tfplan` in memory? | Determines whether the **hard** applied==approved (D-4) holds or downgrades to human-only | workflow spike | DevOps/SRE | **Blocking for the D-4 *hard* guarantee** (not for shipping firebase-dev/plan-only)
- **Q-2** | Secret-at-rest home for third-party statics: GitHub Environment secrets (baseline) vs Architect's "GitHub holds ~zero — fetch from GCP Secret Manager/SOPS via WIF" | Reduces GitHub-resident crown-jewels | cost/ops spike | Security/DevOps | No (optimization)
- **Q-3** | Is a second human reviewer possible, or is single-maintainer self-approval the accepted gate? | "approval" semantics under `required_approving_review_count=0` | governance decision | operator | No (accepted risk R-8-equivalent)
- **Q-4** | firebase-dev reviewer-gated or fast-path (`development` env)? | DX vs consistency | decision | DevOps | No
...
- **R-7** | applied==approved downgrades if job boundary drops the tfplan across approval (A-4/Q-1) | Med | Med | validate at `/upstream`; fallback human-review + no-destroy guard | spike | Architect
- **R-8** | Single-maintainer self-approval = no true four-eyes | High | Med | Environment gate as audit/speed-bump; accepted risk | — | operator
...
`/upstream engineering/2026-07-14-terraform-auto-apply-council/2026-07-14-council-record-v1.md`
(resolve **Q-1** early — it decides whether D-4's applied==approved is tool-enforced or human-only; and **A-3** — the GitHub App token path, which gates the mono-root.)
